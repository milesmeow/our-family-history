...
export async function sendInvitationEmail({
  ...
  const inviterText = inviterName ? `${inviterName} has` : "A family member has";
  const roleText = role === "ADMIN" ? "administrator" : role.toLowerCase();
  ...
  const { data, error } = await resend.emails.send({
    ...
    to,
    subject: "You're invited to join our Family History",
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #374151; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #ffffff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 32px;">
            ...
            <p style="margin: 0 0 16px;">
              ${inviterText} invited you to join our family history app as a <strong>${roleText}</strong>.
            </p>
            ...
            <div style="background: #fef3c7; border-radius: 8px; padding: 16px; margin-top: 24px;">
              <p style="margin: 0; font-size: 14px; color: #92400e;">
                <strong>Note:</strong> This invitation expires on ${expiresText}. If you didn't expect this invitation, you can safely ignore this email.
              </p>
            </div>
          </div>

          <p style="text-align: center; color: #9ca3af; font-size: 12px; margin-top: 24px;">
            This is an automated email from Family History App.
          </p>
        </body>
      </html>
    `,
    text: `
You're Invited to Family History!

${inviterText} invited you to join our family history app as a ${roleText}.
